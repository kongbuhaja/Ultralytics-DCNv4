...
from .conv import DConv, Conv, DWConv, GhostConv, LightConv, RepConv, autopad
...
class DC2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(DBottleneck(self.c, self.c, shortcut, g, k=(3, 3), e=1.0, de=0.95) for _ in range(n))
...
class DCIB(nn.Module):
    """
    Conditional Identity Block (DCIB) module.
...
class DC2fCIB(C2f):
    """
    DC2fCIB class represents a convolutional block with DC2f and DCIB modules.
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, lk=False, g=1, e=0.5):
        """Initializes the module with specified parameters for channel, shortcut, local key, groups, and expansion."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(DCIB(self.c, self.c, shortcut, e=1.0, lk=lk) for _ in range(n))
...
class DC3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, dc3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            DC3k(self.c, self.c, 2, shortcut, g) if dc3k else DBottleneck(self.c, self.c, shortcut, g, de=1.0) for _ in range(n)
        )
...
class DC3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        self.m = nn.Sequential(*(DBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0, de=1.0) for _ in range(n)))
...
class PSD(nn.Module):
    def __init__(self, c1, c2, e=0.5):
        ...
        self.attn = DConv(self.c, self.c, k=3, dk=1, e=0.7)
...
class PSDBlock(nn.Module):
    def __init__(self, c, k=3, shortcut=True):
        super().__init__()
        self.attn = DConv(c, c, k=k, dk=1, e=0.7)
